...
def make_terachem_input(
    ...
    scrdir = os.path.join(workdir, f"tc_scr.{prefix}")
    tc_kwargs["scrdir"] = scrdir
...
def make_terachem_job_array(
    ...
    Args:
        ...
        workdir: directory where the outputs from running will be dumped
    ...
    for i, xyz in enumerate(xyzs):
        ...
        log_path = os.path.join(
            workdir, f"stdout_{os.path.basename(input_path).replace('.in', '.log')}"
        )
